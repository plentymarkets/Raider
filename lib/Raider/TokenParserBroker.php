 ...
 */
class Raider_TokenParserBroker implements Raider_TokenParserBrokerInterface
{
    protected $parser;
    protected $parsers = [];
    protected $brokers = [];
    ...
    /**
     * @param array|\Traversable $parsers                 A \Traversable of Raider_TokenParserInterface instances
     * @param array|\Traversable $brokers                 A \Traversable of Raider_TokenParserBrokerInterface instances
    ...
    public function __construct($parsers = [], $brokers = [], $triggerDeprecationError = true)
    {
        ...
        foreach ($parsers as $parser) {
            if (!$parser instanceof TokenParserInterface) {
                throw new \LogicException('$parsers must a an array of Raider_TokenParserInterface.');
            }
            $this->parsers[$parser->getTag()] = $parser;
        }
        foreach ($brokers as $broker) {
            if (!$broker instanceof Raider_TokenParserBrokerInterface) {
                throw new \LogicException('$brokers must a an array of Raider_TokenParserBrokerInterface.');
            }
            $this->brokers[] = $broker;
        }
    }
